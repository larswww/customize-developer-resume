...
export type StepPromise = () => Promise<any>;
...
export class WorkflowEngine {
  ...
  constructor(
    ...
    try {
      ...
      logDebug("Error initializing Anthropic client", { error: error instanceof Error ? error.message : String(error) });
    }
    ...
    try {
      ...
      logDebug("Error initializing OpenAI client", { error: error instanceof Error ? error.message : String(error) });
    }
    ...
    try {
      ...
      logDebug("Error initializing Gemini client", { error: error instanceof Error ? error.message : String(error) });
    }
  }
        ...
        try {
          ...
          if (index > 0) {
            ...
            logDebug(`Context after previous step:`, {
              hasIntermediateResults: Object.keys(context.intermediateResults).length > 0,
              availableKeys: Object.keys(context.intermediateResults),
              previousStepResult: context.intermediateResults[prevStep.id] ? "Present" : "Missing"
            });
          }
          ...
          let prompt: string;
          try {
            prompt = typeof step.prompt === 'function' 
              ? step.prompt(context)
              : step.prompt;
            logDebug(`Generated prompt for step ${step.id} (length: ${prompt.length} chars)`);
          } catch (error) {
          ...
          try {
            ...
            logDebug(`Received response from ${step.provider} for step ${step.id}`);
          } catch (error) {
            logDebug(`Error from AI provider ${step.provider} for step ${step.id}`, { 
              error: error instanceof Error ? error.message : String(error),
          ...
          try {
            ...
            logDebug(`Error transforming result for step ${step.id}`, { 
              error: error instanceof Error ? error.message : String(error),
              responseText: response.text.substring(0, 200) + (response.text.length > 200 ? '...' : '')
            });
            throw error;
          }
          ...
          try {
            ...
            logDebug(`Note: Could not store step result for API access (might be expected): ${step.id}`);
          ...
          throw new Error(`Error in workflow step ${step.id} (provider: ${step.provider}): ${errorMessage}`);
        }
      };
    });
  ...
  async execute(
    ...
    for (const step of this.steps) {
      logDebug(`Executing step ${step.id} (${completedSteps + 1}/${totalSteps})`);
      try {

...
function WorkflowStepItem({ step, index, result, status }: WorkflowStepItemProps) {
  ...
  return (
    <Collapsible 
      ...
      defaultOpen={false}
      className="mb-4"
      titleClassName="p-4 rounded-t-lg"
...
export function WorkflowSteps({ 
  ...
  // Create two arrays for left and right columns
  const leftColumnSteps = stepsToRender.filter((_, index) => index % 2 === 0);
  const rightColumnSteps = stepsToRender.filter((_, index) => index % 2 === 1);

  return (
    <div className="mb-8">
      <div className="flex flex-col md:flex-row gap-6">
        <div className="md:w-1/2 flex flex-col">
          {leftColumnSteps.map((step, idx) => (
            <WorkflowStepItem
              key={step.id}
              step={step}

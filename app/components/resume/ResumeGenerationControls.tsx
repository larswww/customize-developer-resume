...
export function ResumeGenerationControls({
	...
	const allSourceTextsPresent = sourceSteps.every(
		(step) => sourceTexts[step.id] && sourceTexts[step.id].trim() !== "",
	);
	const generateDisabled =
		isSubmitting || isGenerating || !allSourceTextsPresent;
	...
	return (
		<div className="bg-white shadow-sm rounded-lg p-6 mb-8 border border-gray-200">
			...
			<p className="text-sm text-gray-500">
				Confirm your details and text inputs above, then use the button to
				generate the structured resume sections.
			</p>

			{!allSourceTextsPresent && (
				<div className="mt-4 p-3 bg-yellow-50 text-yellow-700 text-sm rounded-md border border-yellow-100">
					<p className="text-center">
						Required text missing for: {missingTexts || "Unknown sections"}.
						Please fill all sections before generating.
					</p>
				</div>
			)}
		</div>

...
const ensureHttps = (url: string): string => {
  ...
  if (url.includes('.')) { // Basic check if it looks like a domain
     return `https://${url}`;
  }
  ...
  if (url.includes('/')) { // Simple check for path like linkedin
      return `https://${url}`; 
  }
    ...
    <div className="p-8 font-sans text-sm bg-white">
      ...
      {employmentHistory && employmentHistory.length > 0 && (
        <section className="mb-4">
          <h2 className="text-xl font-semibold border-b pb-1 mb-4 text-gray-800">Experience</h2>
          {employmentHistory.map((employment, empIndex) => (
            <div key={`emp-${employment.employer}-${empIndex}`} className="mb-6 pb-3 border-b border-gray-100 last:border-b-0">
              <h3 className="text-lg">
                 <span className="font-normal text-gray-600">{employment.title}</span>
                 <span className="text-gray-800 font-semibold"> at {employment.employer}</span>
              </h3>
              <p className="text-xs text-gray-500 mb-3">{employment.dates} | {employment.location}</p>

              {employment.projects && employment.projects.length > 0 && (
                  <div className="mt-1 space-y-4">
                      {employment.projects.map((project, projectIndex) => (
                          <div key={`p-${empIndex}-${project.client}-${projectIndex}`} className="pb-2 last:pb-0">
                                  ...
                                  {project.skillsUsed && project.skillsUsed.length > 0 && (
                                      <ul className="flex flex-wrap gap-x-1.5 gap-y-1 text-xs">
                                          {project.skillsUsed.map((skill, skillIndex) => (
                                              <li key={`p${empIndex}-${projectIndex}-s${skillIndex}-${skill}`} className="bg-gray-100 text-gray-600 px-1.5 py-0 rounded-sm border border-gray-200 text-xs">
                                                  {skill}
                                              </li>
                                          ))}
                                      </ul>
                              ...
                              {project.description && project.description.length > 0 && (
                                  ...
                                  {project.description.map((desc, descIndex) => <li key={`p${empIndex}-d${projectIndex}-${descIndex}-${desc.slice(0,10)}`}>{desc}</li>)}
                                  </ul>
                              )}
      ...
      {education && education.length > 0 && (
         <section className="mb-4">
           <h2 className="text-xl font-semibold border-b pb-1 mb-3 text-gray-800">Education</h2>
           {education.map((edu, eduIndex) => (
             <div key={`edu-${edu.institution}-${edu.degree}-${eduIndex}`} className="mb-2 last:mb-0">
               <h3 className="font-semibold text-base text-gray-800">{edu.degree}</h3>
               <p className="text-xs text-gray-500"> {edu.institution} | {edu.dates} | {edu.location}</p>
             </div>
           ))}
         </section>
      )}

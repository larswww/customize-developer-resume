			...
			<div className="flex flex-row h-full flex-grow">
				...
				>
					...
					>
						...
						<div className="space-y-0.5">
							<p className="flex items-center text-sm">
								<span className="mr-2">📍</span> <TextWrap text={contactInfo.location} name="contactInfo.location" label="Location" />
							</p>
							<p className="flex items-center text-sm">
								<span className="mr-2">📞</span> <TextWrap text={contactInfo.phone} name="contactInfo.phone" label="Phone" />
							</p>
							<p className="flex items-center text-sm">
					...
					<div className="p-5 pt-4">
						{education && (
							<>
								<h2 className="text-xl font-bold uppercase mb-2"><TextWrap text="EDUCATION" name="education.title" label="Education Section Title" /></h2>
								<p className="font-bold text-sm"><TextWrap text={education.degree} name="education[0].degree" label="Degree" /></p>
								<p className="text-sm"><TextWrap text={education.institution} name="education[0].institution" label="Institution" /></p>
								<p className="text-sm"><TextWrap text={education.dates} name="education[0].dates" label="Dates" /></p>
								<p className="text-sm"><TextWrap text={education.location} name="education[0].location" label="Location" /></p>
							</>
						)}
							...
							renderItem={(skillCategory, categoryIndex) => (
								<div className="mb-3">
									<p className="font-bold text-gray-700 text-sm uppercase">
										<TextWrap text={skillCategory.category} name={`skills[${categoryIndex}].category`} label="Skill Category" />
									</p>
									<div className="space-y-0">
										<ArrayRenderer
											...
											renderItem={(item, itemIndex) => (
												<p className="text-sm">
													<TextWrap text={item.name} name={`skills[${categoryIndex}].items[${itemIndex}].name`} label="Skill Name" />
													{item.context && (
														<span className="text-xs text-gray-500 ml-1">
															(<TextWrap text={item.context} name={`skills[${categoryIndex}].items[${itemIndex}].context`} label="Skill Context" />)
														</span>
													)}
												</p>
											)}
						...
						{otherInfo?.items?.length ? (
							<>
								<h2 className="text-xl font-bold uppercase mt-5 mb-2">
									<TextWrap text={otherInfo.title || "OTHER"} name="otherInfo.title" label="Other Info Section Title" />
								</h2>
								<ArrayRenderer
									...
									renderItem={(item, index) => (
										<p className="text-sm">
											<TextWrap text={item} name={`otherInfo.items[${index}]`} label="Other Info Item" />
										</p>
									)}
								/>
							</>
								...
								<ArrayRenderer
									...
									renderItem={(lang, index) => (
										<span className="w-5 h-5">
											<TextWrap text={lang} name={`languages[${index}]`} label="Language" />
										</span>
									)}
								/>
							</div>
								...
								<div className="mb-1">
									<div className="flex justify-between items-baseline mb-0.5">
										<span className="font-bold text-lg text-gray-900">
											<TextWrap text={job.title} name={`workExperience[${jobIndex}].title`} label="Job Title" />
										</span>
										<span className="text-sm text-gray-600 text-right flex-shrink-0 ml-4">
											<TextWrap text={job.dates} name={`workExperience[${jobIndex}].dates`} label="Job Dates" />
										</span>
									</div>
									<p className="text-sm text-gray-600">
										<TextWrap text={job.company} name={`workExperience[${jobIndex}].company`} label="Company" />, <TextWrap text={job.location} name={`workExperience[${jobIndex}].location`} label="Job Location" />
									</p>
								</div>
								...
								<ArrayRenderer
									...
									renderItem={(desc, descIndex) => (
										<p className="mb-2 text-gray-800">
											<TextWrap text={desc} name={`workExperience[${jobIndex}].description[${descIndex}]`} label="Job Description" />
										</p>
									)}
								/>
									...
									<ul className="list-disc pl-5 mt-2 text-gray-700">
										<ArrayRenderer
											...
											renderItem={(highlight, highlightIndex) => (
												<li className="mb-1">
													<TextWrap text={highlight} name={`workExperience[${jobIndex}].highlights[${highlightIndex}]`} label="Job Highlight" />
												</li>
											)}
										/>
									</ul>

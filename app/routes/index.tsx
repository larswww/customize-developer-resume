...
import { Await, useLoaderData, useAsyncError } from 'react-router';
...
export async function action({ request }: ActionFunctionArgs) {
  ...
  try {
    if (mode === 'serverOnly') {
      // For server-side execution (all at once)
      ...
      const stepResults: Record<string, Promise<any>> = {};
...
export default function Index() {
  ...
  const actionData = useActionData<{ 
    ...
    error?: string,
    mode?: string,
    stepPromises?: Record<string, Promise<any>>,
    totalSteps?: number
  }>();
  ...
  const renderWorkflowSteps = () => {
    if (!actionData?.stepPromises || !actionData.success || actionData.mode !== 'withSteps') {
      return null;
    }
          ...
          <div key={step.id} className="mb-4">
            ...
            <Suspense fallback={
              <div className="ml-10 p-3 border rounded bg-gray-50">
                ...
                <div className="mt-2 text-sm text-gray-500">Processing {getStepName(step.id)}...</div>
              </div>
            }>
              <Await 
              ...
              >
                {(result) => (
                  <div className="ml-10 p-3 border rounded bg-green-50">
                    {step.id === finalStepId ? (
                      <div>
                        <h3 className="font-medium mb-2">Your Generated Resume:</h3>
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      {actionData?.success && actionData.result && actionData.mode === 'serverOnly' && (

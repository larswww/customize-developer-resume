...
const targetJsonSchema = JSON.stringify(zodToJsonSchema(SimpleConsultantCoreDataSchema), null, 2);
...
export const workflowSteps: WorkflowStep[] = [
  {
    ...
    systemPrompt:
      `You strictly extract user requirements for the target role into a structured JSON object based on the provided job description. Do not add assumptions. Include only explicitly stated information. If stated, include fields like:
...
Output strictly the JSON object and nothing else.`,
  ...
  },
  {
    ...
    description: "Score and rank parsed work history items based on relevance to the job description.",
    provider: "anthropic", // Top-level provider
    options: {
      provider: "anthropic", // Required inside options by type
...
Output strictly the augmented JSON object with relevance scores and nothing else.`,
  ...
  },
  {
    ...
    description: "Generate tailored resume summary and employment sections based on top-ranked experiences.",
    provider: "anthropic", // Top-level provider
    options: {
      provider: "anthropic", // Required inside options by type
    ...
    systemPrompt:
      `Generate core content for a targeted resume using the provided inputs. Focus on the highest-scoring experiences (relevance score >= 7 or top 3-5 experiences). Create:
...
Output strictly a JSON object containing 'summary' and 'employmentHistory' keys, following the specified structure.`,
  ...
  },
  {
    ...
    description: "Format the generated content into the final JSON structure matching the SimpleConsultantCoreDataSchema.",
    provider: "anthropic", // Top-level provider
    options: {
      provider: "anthropic", // Required inside options by type
